...
def load_model():
    global lstm_model, lstm_tok, svc_model, svc_vect, graph
...
def predict():
    try:
        ...
        if model == 'svc':
            ...
            return jsonify({"result": s_predict, "probability": s_predict_proba})
        elif model == 'lstm':
            l_predict, l_predict_proba = lstm_predict(l_text)
            return jsonify({"result": l_predict, "probability": l_predict_proba})
        else:
            s_predict, s_predict_proba = svc_predict(l_text)
            l_predict, l_predict_proba = lstm_predict(l_text)
